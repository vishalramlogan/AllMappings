...
@Component({
  selector: 'app-wb1-p-m',
  templateUrl: './wb1-p-m.component.html',
  styleUrls: ['./wb1-p-m.component.scss']
})
export class WB1PMComponent implements OnInit {
  Message: string;
  ...
  ngOnInit(): void {
    this.webbrowsingsSub = this.webbrowsingService.getWeb1UpdateListner().subscribe((webbrowsings: WebBrowsing[])=>{
  ...
  calculate(event){
    ...
    this.webbrowsingService.addWB1PSMdb(expectedSessionTime,sessionTime)
  }
