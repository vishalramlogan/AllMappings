...
exports.AudioStreamingLR_post = function(req,res){
    if (req.body.lossRateIPPackets > 100 || req.body.lossRateIPPackets < 0){
        return res.status(400).json({
            message: 'RTP Packet Loss should be between 0 and 100'
        }); //Bad Request
    }else if(req.body.audioCodec != 'AAC-LC' && req.body.audioCodec != 'AAC-HEv1' && req.body.audioCodec != 'AAC-HEv2' && req.body.audioCodec != 'AMR-NB' && req.body.audioCodec != 'AMR-WB+'){
        return res.status(400).json({
            message: 'Enter a valid Audio Codec from table'
        }); //Bad Request
    }else {    
        var A_BR = req.body.audioBitrateLR,//audioBitrateLR
        AudioFrameLength = req.body.audioFrameLength,//audioFrameLength
        IPPacketAverageBurstLength = req.body.averageBurstIP, // averageBurstIP
        MaximumofDataSizeperPacket = req.body.maxSizeAudioStream,
        IPpacketLossRate = req.body.lossRateIPPackets, // lossRateIPPackets
        NumberofAudioFramesperPacket = req.body.numAudioFrames, // numAudioFrames
