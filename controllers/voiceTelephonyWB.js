...
exports.VoiceTelephonyWB_post = function(req,res){
    if (req.body.speechDelay > 1000){
        return res.status(400).json({
            message: 'Speech Delay should be less than 1000'
        }); //Bad Request
    }else if(req.body.speechPacketLossRate > 20){
        return res.status(400).json({
            message: 'Speech Packet-Loss Rate should be less than 20%'
        }); //Bad Request
    } else{
        var Ts = req.body.speechDelay, // speechDelayNB
        Ppls = req.body.speechPacketLossRate, // speechPacketLossRateNB
        TELR = req.body.talkerEchoLoudness, //talkerEchoLoudnessNB 
        IesWB = req.body.equipmentImpairment, // Equipment Impairment Factor
        Bpls = req.body.packetLossRoubustness; // Packet-Loss Robustness Factor
