...
exports.EModelNarrowband_post = function(req,res){
    if (req.body.packetLossProb > 100 || req.body.packetLossProb < 0){
        ...
        return res.status(400).json({
            message: 'Class of Delay Sensitivity must be either "Default", "Low" or "Very Low"'
        }); //Bad Request
    }else {
        var Nc = 1*req.body.electricCircuitNoise, // electric Cricuti Noise
        Nfor = 1*req.body.noiseFloor, // Noise floor 
        Ps =1*req.body.roomNoiseS, // room Noise Sender
        Pr =1*req.body.roomNoiseR, // room Noise Receiver
        SLR =1*req.body.sLoudnessRating, //sender Loudness Rating
        RLR =1*req.body.rLoudnessRating, //receiver Loudness Rating
        STMR =1*req.body.sidetoneMaskingRating, // sidetone Masking Rating 
        Dr =1*req.body.dFactorR, // d Factor Receiver
        LSTR = (STMR + Dr), // listiner Sidetone Rating SEE If TO REMOVE THIS
        Ds = 1*req.body.dFactorS, // d Factor Sender
        classofDelaySensitivity = req.body.classDelaySensitivity, 
        T = 1*req.body.meanOneWayDelay, // mean One Way Delay
        Ta = 1*req.body.absoluteDelay, // absolute Delay SEE If TO REMOVE THIS
        Tr = 1*req.body.roundTripDelay, // round Trip Delay SEE If TO REMOVE THIS
        TELR =1*req.body.talkerEchoLoudness, // talker echo Loudness Rating
        WEPL=1*req.body.weightedEchoPathLoss, //weighted echo Path Loss
        qdu=1*req.body.qdu, 
        Ie = 1*req.body.equipmentImpairmentFac, // equipment Impairment Factor
        Bpl =1*req.body.packetLossRobustness, //packet Loss Robustness Factor 
        Ppl =1*req.body.packetLossProb, // packet Loss Probability
        BurstR =1*req.body.burstRate,// Burst Ratio
        A = 1*req.body.advantageFactor; // advantage factor
