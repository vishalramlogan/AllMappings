...
exports.VideoTelephony_post = function(req,res){
    if (req.body.videoPacketLoss > 10){
        return res.status(400).json({
            message: 'Video Packet-Loss Rate should be less than 10%'
        }); //Bad Request
    }else if(req.body.videoFrameRate > 30 || req.body.videoFrameRate < 1){
        return res.status(400).json({
            message: 'Video Frame Rate should between 1 to 30'
        }); //Bad Request
    }else if(req.body.combinationVT > 21 || req.body.combinationVT < 1){
        return res.status(400).json({
            message: 'Video Frame Rate should between 1 to 30'
        }); //Bad Request
    }else{
        var Pplv = req.body.videoPacketLoss, // videoPacketLoss
        Frv = req.body.videoFrameRate, //videoFrameRate
        Brv =req.body.videoBitRate, // videoBitRate
        combinationVT = req.body.combinationVT;
