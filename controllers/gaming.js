...
exports.gaming_post = function(req,res){
    if (req.body.packetLoss > 100 || req.body.packetLoss < 0){
        return res.status(400).json({
            message: 'Packet Loss should be between 0 and 100'
        }); //Bad Request
    }else if(req.body.lossSensitivityClass != 'Low' && req.body.lossSensitivityClass != 'High'){
        return res.status(400).json({
            message: 'Loss Sensitivity Case must be Low or High'
        }); //Bad Request
    }else if(req.body.encodingComplexity != 'Low' && req.body.encodingComplexity != 'Medium' && req.body.encodingComplexity != 'High'){
        return res.status(400).json({
            message: 'Encoding Complexity must be Low, Medium or High'
        }); //Bad Request
    }else{    
        var bitrate = 1*req.body.bitrate,
        numPixelsPerVideoFrame = 1*req.body.numPixelsPerVideo,
        PL = 1*req.body.packetLoss, // packet Loss
        FRenc = 1*req.body.encodingFrameRate, // encodingFrameRate
        lossSensitivityClass = req.body.lossSensitivityClass,
        encodingComplecity = req.body.encodingComplexity,
        delay =  1*req.body.delay;
