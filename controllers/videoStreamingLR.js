...
exports.VideoStreamingLR_post = function(req,res){
    if (req.body.packetLoss != 'Yes' && req.body.packetLoss != 'No'){
        return res.status(400).json({
            message: 'Packet Loss must be Yes or No'
        }); //Bad Request
    }else if (req.body.rebuffering != 'Yes' && req.body.rebuffering != 'No'){
        return res.status(400).json({
            message: 'Rebuffering must be Yes or No'
        }); //Bad Request
    }else if (req.body.videoPLC != 'slicing' && req.body.videoPLC != 'freezing'){
        return res.status(400).json({
            message: 'Video PLC must be slicing or freezing'
        }); //Bad Request
    }else if (req.body.videoContentCoding > 5 || req.body.videoContentCoding < 1){
        return res.status(400).json({
            message: 'Video Content Complexity must be between 1 and 5'
        }); //Bad Request
    }else if (req.body.codingCompression > 6 || req.body.codingCompression < 1){
        return res.status(400).json({
            message: 'Other Coding Compression must be between 1 and 6'
        }); //Bad Request
    }else if (req.body.ipPacketLossRate > 1 || req.body.ipPacketLossRate < 0){
        return res.status(400).json({
            message: 'IP Packet Loss Rate must be less than 1'
        }); //Bad Request
    }else if (req.body.ipPacketLoss > 1 || req.body.ipPacketLoss < 0){
        return res.status(400).json({
            message: 'IP Packet Loss normally be less than 1'
        }); //Bad Request
    }else{   
        var packetLoss = req.body.packetLoss,
        rebuffering = req.body.rebuffering,
        Width = req.body.videoWidth, // videoWidth
        Height = req.body.videoHeight, //videoHeight
        videoPLC = req.body.videoPLC,
        ARL = req.body.rebufferingLength, //rebufferingLength in seconds
        NRE = req.body.numRebufferingEvents, //numRebufferingEvents
        MREEF = req.body.rebufferingFactor, // rebufferingFactor
        V_NBR = req.body.numVideos, // numVideos or Variable Number of Buffering Events Rate
        videoContentCoding = req.body.videoContentCoding,
        codingCompression = req.body.codingCompression,
        MeasureTime = req.body.measureTime, //
        V_Burst = req.body.ipPacketLoss, //ipPacketLoss in seconds (noramlly less than 1)
        V_LossRate = req.body.ipPacketLossRate, //ipPacketLossRate (less than 1)
        GopLength = req.body.gopLength, //gopLength
        V_BR = req.body.videoBitrate, // videoBitrate
