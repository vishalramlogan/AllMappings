...
exports.VoiceTelephonyNB_post = function(req,res){
    if (req.body.speechDelayNB > 1000){
        return res.status(400).json({
            message: 'Speech Delay should be less than 1000'
        }); //Bad Request
    }else if(req.body.speechPacketLossRateNB > 20){
        return res.status(400).json({
            message: 'Speech Packet-Loss Rate should be less than 20%'
        }); //Bad Request
    }else if(req.body.combination<1 || req.body.combination[0]>7 ){
        return res.status(400).json({
            message: 'Combination should be between 1 to 7'
        }); //Bad Request
    } else{    
        var Ts = req.body.speechDelayNB, // speechDelayNB
        Ppls = req.body.speechPacketLossRateNB, // speechPacketLossRateNB
        TELR = req.body.talkerEchoLoudnessNB, //talkerEchoLoudnessNB 
        combination = req.body.combination; // combination
