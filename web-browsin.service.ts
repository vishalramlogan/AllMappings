...
import { WebBrowsing,WebBrowsing2,WebBrowsingSTE,WebBrowsingSA } from './webbrowsing.model'; 
...
export class WebBrowsinService {
  ...
  private webbrowsingSAs: WebBrowsingSA[] = [];
  private webbrowsingSAsAdd = new Subject<WebBrowsingSA[]>();
  ...
  MessageSA = new EventEmitter<string[]>();
  ...
  addWB1PSSAdb(expectedSessionTime: number[], sessionTime: number[]){
    const username = localStorage.getItem('theUser');
    const webbrowsingSA: WebBrowsingSA = {expectedSessionTime1PS: expectedSessionTime,sessionTime1PS: sessionTime};
    const headers = {'Authorization': `Bearer ${this.token}`};

       this.http.post<{MOS: []}>(("http://localhost:3000/api/users/"+username+"/web1PageSession/ExpectedSA"), webbrowsingSA ,
       { headers }).subscribe(
         (messageData) => {
           this.webbrowsingSAs.push(webbrowsingSA);
           this.webbrowsingSAsAdd.next([...this.webbrowsingSAs]);
           this.MessageSA.emit(messageData.MOS);
         },
         error => {
           if (error.status != 200) {
             alert(error.error.message);
  ...
  getWeb1UpdateListner(){
    return this.webbrowsingsAdd.asObservable();
  }
  getWeb1SAUpdateListner(){
    return this.webbrowsingSAsAdd.asObservable();
  }
  getWeb2UpdateListner(){
    return this.webbrowsingsAdd2.asObservable();
  }
  getWebSTEUpdateListner(){
    return this.webbrowsingsAddSTE.asObservable();
  }
}
